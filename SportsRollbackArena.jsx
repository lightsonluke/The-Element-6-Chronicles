...
export default function SportsRollbackArena({ match, players, settings = {}, onEnd }) {
  ...
  if(result) return <div className="text-center space-y-5"><h2 className="text-4xl font-heading text-accent">{result.forfeit ? 'OPPONENT LEFT · YOU WIN!' : `TEAM ${result.winnerTeam} WINS!`}</h2><button onClick={finish} className="px-6 py-3 bg-primary rounded font-heading">CONTINUE</button></div>;
  return <div className="w-full max-w-5xl text-center"><div className="flex justify-between mb-2"><button onClick={quit} className="px-3 py-1 bg-secondary rounded text-xs">FORFEIT</button><p className="font-heading text-accent">{status}</p></div><div className="relative"><canvas ref={canvasRef} width={WIDTH} height={HEIGHT} className="w-full border-2 border-primary rounded-lg" />{syncing && <div className="absolute inset-0 grid place-items-center bg-black/70 font-heading text-accent text-xl">SYNCING MATCH…</div>}</div><p className="text-xs text-muted-foreground mt-2">Automatic checksum checks keep every player on the same confirmed frame.</p></div>;
}
