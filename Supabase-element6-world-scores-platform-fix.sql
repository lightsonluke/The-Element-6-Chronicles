-- Element 6 world activity leaderboard persistence.
-- Safe to run after the existing element6_world_scores setup.
begin;
...
create policy element6_world_scores_read on public.element6_world_scores
...
begin
  ...
  if p_mode not in ('parkour','zipline','rockclimb') then raise exception 'Unknown leaderboard mode'; end if;
  ...
  select coalesce(
    ...
    split_part(email,'@',1),
    'Player'
  ) into n from auth.users where id=u;

  select score into existing
  from public.element6_world_scores
  where user_id=u and mode=p_mode;

  -- Parkour/zipline: larger is better. Rock climbing: smaller time is better.
  ...
  return jsonb_build_object('updated',should_replace);
end;
$$;
