...
// Iconsout's React Unicons stay inside the same fixed inline box that the
...
export default function GameIcon({ emoji = '', size = 16, color = 'currentColor', className = '', style = {} }) {
  ...
  if (!Icon) return <span aria-hidden="true" className={`inline-flex shrink-0 align-middle leading-none ${className}`} style={{ width: size, height: size, ...style }} />;
  return <span aria-hidden="true" className={`inline-flex shrink-0 items-center justify-center align-middle leading-none ${className}`} style={{ width: size, height: size, ...style }}><Icon size={size} color={color} /></span>;
}
