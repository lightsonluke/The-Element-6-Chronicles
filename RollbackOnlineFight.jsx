...
export default function RollbackOnlineFight({
  ...
  useEffect(() => {
    ...
    const finishMatch = (winningRole, proofState = null) => {
      if (finished) return;
      finished = true;
      resultProofRef.current = proofState ? {
        winnerRole: winningRole,
        finalFrame: proofState.resultFrame,
        checksum: checksumState(proofState),
    ...
    const start = async () => {
      try {
        transport = new SupabaseRollbackTransport({ matchId, playerId, peerPlayerId: opponentPlayerId, mode });
        session = new RollbackSession({
          ...
          onDesync: () => {
            ...
            else transport?.sendControl('resync-request').catch(() => {});
          },
          onFrame: ({ state }) => {
        ...
        const loop = now => {
          ...
          lastTime = now;
          let steps = 0;
          while (!resyncing && accumulator >= FRAME_MS && steps < 6) {
            ...
            if (state.winner && Number.isInteger(state.resultFrame) && session.confirmedFrame >= state.resultFrame - 1) {
              const proofState = session.getStateAtFrame(state.resultFrame);
              if (proofState) { finishMatch(state.winner, proofState); break; }
            }
