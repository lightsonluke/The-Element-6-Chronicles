...
async function persistClip() {
  ...
  if (!result?.blob || result.blob.size < 128) {
    showToast('CLIP IS NOT READY YET — TRY AGAIN IN A MOMENT');
    return false;
  }
  ...
  showToast(
    `CLIP SAVED — ${Math.max(1, Math.round(result.duration))} SECONDS — MP4 60FPS`
  );
