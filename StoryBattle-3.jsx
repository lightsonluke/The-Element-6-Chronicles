...
export default function StoryBattle({ heroId, villainId, enemyIds, allyIds, stageId, difficulty = 'hard', battleTitle, storyStocks, storyTime, narrativeLoss = false, onEnd, equippedAccessories = {}, equippedSkins = {}, equippedShikigami = {}, settings = {}, equippedEmotes = {} }) {
  ...
  useEffect(() => {
    ...
    // ── Stage setup ──
    let mapId, platforms;
    if (stageId && STORY_STAGE_PLATFORMS[stageId]) {
      mapId = stageId;
      platforms = STORY_STAGE_PLATFORMS[stageId];
    } else if (isMulti) {
    ...
    f1.grounded = true;
    if (storyStocks) f1.stocks = storyStocks;
    f1.team = 'player';
    playerTeam.push(f1);
    ...
    const loop = (now) => {
      ...
      if (gameRef.current.timer <= 0 || !playerAlive || !enemyAlive) {
        ...
        setResult(narrativeLoss ? 'narrative' : (win ? 'win' : 'lose'));
        return;
      }
      ...
      {result && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70 rounded-lg gap-4">
          <span className={`text-4xl font-heading ${result === 'narrative' || result === 'win' ? 'text-accent' : 'text-destructive'}`}>
            {result === 'narrative' ? 'STORY CONTINUES' : (result === 'win' ? 'VICTORY!' : 'DEFEATED')}
          </span>
          <button
            onClick={() => onEnd(result === 'narrative' ? 'narrative' : result === 'win')}
