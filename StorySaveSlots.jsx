...
export default function StorySaveSlots({ slots=[null,null,null], onSelect, onDelete, onBack }) {
  ...
  return <div className="w-full max-w-4xl flex flex-col gap-5 text-white">
    ...
    <div className="grid gap-3 max-h-[70vh] overflow-y-auto pr-1">{Array.from({length:5},(_,i)=>{const data=slots[i];const h=getHero(data);const pct=calculateStoryProgress(data||{});const confirming=confirmDelete===i;return <div key={i} className={`rounded-2xl border p-4 flex items-center gap-4 ${data?'border-white/15 bg-black/45':'border-white/8 bg-black/25'}`}>
