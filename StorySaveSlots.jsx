...
import { STORY_SAVE_SLOTS, STORY_BOOKS, calculateStoryProgress } from './storyModeOverhaul.js';
...
export default function StorySaveSlots({ slots=[null,null,null], onSelect, onDelete, onBack }) {
  ...
  return <div className="w-full max-w-4xl flex flex-col gap-5 text-white">
    ...
    <div className="grid gap-3">{Array.from({length:STORY_SAVE_SLOTS},(_,i)=>{const data=slots[i];const h=getHero(data);const pct=calculateStoryProgress(data||{});const confirming=confirmDelete===i;return <div key={i} className={`rounded-2xl border p-4 flex items-center gap-4 ${data?'border-white/15 bg-black/45':'border-white/8 bg-black/25'}`}>
      ...
      <div className="flex-1 min-w-0">{data?<><div className="font-heading text-sm truncate">{nickname(h)}</div><div className="text-[11px] text-white/45 mt-0.5">Book {data?.currentBook||1} • {STORY_BOOKS[(data?.currentBook||1)-1]?.title||'DAWN OF HEROES'} • Progress: {pct}%</div><div className="text-[10px] text-white/30 mt-1">Location: {data?.currentArea||'Beginning'} • Shards: {data?.collectedShards||0}</div><div className="mt-1.5 h-1 bg-white/10 rounded overflow-hidden"><div className="h-full bg-white" style={{width:`${pct}%`}}/></div></>:<div className="text-sm text-white/25">EMPTY — + NEW SAVE</div>}</div>
