...
// Resolve a character's name/id for display on the leaderboard.
...
export default function ParkourLeaderboard({ onBack, customCharsData = {} }) {
  const [tab, setTab] = useState('global'); // global | friends | search
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        let all = [];
        try { all = await loadWorldLeaderboard('parkour'); }
        catch { all = await db.entities.ParkourScore.list('-distance', 200); }
        if (cancelled) return;
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-3xl">
      <div className="flex justify-between items-center w-full">
        <h2 className="text-2xl font-heading text-accent tracking-wider"><GameIcon emoji="🏙️" size={14} /> PARKOUR LEADERBOARD</h2>
      ...
      <div className="w-full rounded-xl border border-border bg-card overflow-hidden">
        <div className="grid grid-cols-[60px_1fr_120px_100px_90px] gap-2 px-4 py-2 bg-secondary/60 text-[10px] font-heading uppercase tracking-wider text-muted-foreground">
          <span>Rank</span><span>Player</span><span>Character</span><span className="text-right">Distance</span><span className="text-right">Date</span>
        </div>
        <div className="max-h-[52vh] overflow-y-auto">
          {loading ? (
            <div className="p-6 text-center text-sm text-muted-foreground font-body">Loading rankings...</div>
          ) : filtered.length === 0 ? (
            <div className="p-6 text-center text-sm text-muted-foreground font-body">No scores yet — be the first!</div>
          ) : filtered.map((e, i) => (
            <div key={e.id || i} className="grid grid-cols-[60px_1fr_120px_100px_90px] gap-2 px-4 py-2 text-xs font-body border-t border-border/40 items-center hover:bg-accent/5">
              <span className="font-heading text-accent">#{e.rank || (i + 1)}</span>
              <span className="truncate text-foreground">{e.user_name || 'Unknown'}</span>
              <span className="truncate text-muted-foreground">{e.char_name || charName(e.char_id, customCharsData)}</span>
              <span className="text-right font-heading text-primary">{e.distance}m</span>
      ...
      <p className="text-[10px] text-muted-foreground font-body text-center">Scores update automatically after every completed run. Split City Parkour is single-player and offline — the leaderboard syncs when you finish a run.</p>
    </div>
  );
}