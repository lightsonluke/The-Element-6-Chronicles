...
export default function ZiplineLeaderboard({ onBack, customCharsData = {} }) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-3xl">
      <div className="flex justify-between items-center w-full">
        <h2 className="text-2xl font-heading text-accent tracking-wider"><GameIcon emoji="🪢" size={14} /> ZIPLINE LEADERBOARD</h2>
      ...
      <div className="w-full rounded-xl border border-border bg-card overflow-hidden">
        <div className="grid grid-cols-[60px_1fr_120px_110px_90px] gap-2 px-4 py-2 bg-secondary/60 text-[10px] font-heading uppercase tracking-wider text-muted-foreground">
        ...
        <div className="max-h-[52vh] overflow-y-auto">
          {loading ? (
            ...
            <div className="p-6 text-center text-sm text-muted-foreground font-body">No runs yet — be the first to ride the ziplines!</div>
          ) : filtered.map((e, i) => (
            <div key={e.id || i} className="grid grid-cols-[60px_1fr_120px_110px_90px] gap-2 px-4 py-2 text-xs font-body border-t border-border/40 items-center hover:bg-accent/5">
      ...
      <p className="text-[10px] text-muted-foreground font-body text-center">Ranked by longest distance. The leaderboard syncs automatically after every run.</p>
    </div>
  );
}